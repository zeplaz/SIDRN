...
class  imagkjz_RGB{
    ...
    public:
      ...
      inline void write_head(std::ostream out_stream, img_header const& out_head)
       ...
       inline  void read_pix_rgb_raw(std::istream& in_img_stream,std::vector<std::uint8_t>* const con_raw_pix_prt)
         {
        ...
        inline void write_raw_pix_data(std::ostream& out_strem,std::uint8_t const* const raw_pixdata, std::size_t const size)
         ...
         inline void write_image(std::ostream& out_stream,
                                 std::size_t const x_with, std::size_t const y_high)
           {
               std::unit8_t const* const pixdata = raw_img_rgb_vec.data();
               ...
               image_realm::write_raw_pix_data(out_stream,pixdata,out_header.x_with,
                                               out_header.y_high*3);
            }
