    ...
    void imagkjz_RGB::load_image_RGB(std::istream& input_strm)
     {
        std::vector<std::uint8_t>* const pixel_raw_RGB  = &raw_img_rgb_vec;
                              //     ::size_t* const hy,
        auto img_header = image_realm::read_header(input_strm);

        pixel_raw_RGB->resize((img_header.widthx)*(img_header.highty)*3);
        img_size = img_header.widthx*img_header.highty;
        image_realm::read_pix_rgb_raw(input_strm,pixel_raw_RGB);
      ...
      void imagkjz_RGB::linilzatfeture_space()
        {
            ...
            //.image_intcity_struc.colour_rgb_Raw
              image_intcity_struc.img_colour_intisity_double[0]   = reinterpret_cast<double>(raw_img_rgb_vec.at(Pix_id));
              image_intcity_struc.img_colour_intisity_double[1] = reinterpret_cast<double>(raw_img_rgb_vec.at(Pix_id+1));
              image_intcity_struc.img_colour_intisity_double[2] = reinterpret_cast<double>(aw_img_rgb_vec.at(Pix_id+2));
