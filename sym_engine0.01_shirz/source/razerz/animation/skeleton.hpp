...
class skeleton
{
  ...
  std::unordered_map<std::string,joint>::const_iterator iter_joint;
  ...
  skeleton()
  {
    ...
    animation_map.insert("idel",nullptr);
    anim_loop = false;
    ...
    for(int i =0; i<MAX_JOINTZ; i++)
    {
      ...
      else
      {
        ...
        for (std::map<std::string,joint>::iterator it=jointz_map.begin(); it!=jointz_map.end(); ++it)
        {
          ...
          glm::mat4 concat_tranz = (temp_joint->get_parent_transform()* AiToGLMMat4(temp_joint->node->mTransformation));
...
void update()
{
 ...
 time += globalz::frame_delta*0.001f;
