...
class wavefornt_parser2
{
  ...
  public :
  void return_data(std::vector < glm::vec3 > & out_vertices,
...
/*
    std::vector<mesh_vertex> return_mesh_vec()
    { size_t v_size = vertices.size();
      size_t uv_size = uvs.size();
      size_t n_size = normals.size();
      if( v_size != uv_size & v_size !=n_size)
      {std::cout <<  "somethings wrong inproper number of data\n";}
      std::vector<mesh_vertex> mesh_vec;

      for(size_t i = 0; i<v_size;i++)
      {
        mesh_vertex temp_mv;
        temp_mv.v_position  =vertices.at(i);
        temp_mv.v_textcord  =uvs.at(i);
        temp_mv.v_normal    =normals.at(i);
        mesh_vec.push_back(temp_mv);
      }
      return mesh_vec;
    }*/
